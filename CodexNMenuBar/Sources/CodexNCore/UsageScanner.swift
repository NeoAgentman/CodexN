...
public struct CodexUsageScanner {
    ...
    private func scanProfile(_ profile: CodexUsageProfile, date: Date, calendar: Calendar) throws -> CodexUsageTotals {
        ...
        for file in collectUsageFiles(codexHome: profile.codexHome, date: date, calendar: calendar) {
            totals.add(try scanUsageFile(file, date: date, calendar: calendar))
        }
        return totals
    }

    private func collectUsageFiles(codexHome: URL, date: Date, calendar: Calendar) -> [URL] {
    ...
    private func collectFlatJSONLFiles(
        ...
        let startKey = Self.dayKey(for: startDate, calendar: calendar)
        let endKey = Self.dayKey(for: endDate, calendar: calendar)
